import { getBlockTimestamp, getDateFromTimestamp, getGracePeriod } from "./helpers";
...
interface SarcophagusDataSimple {
  id: string;
  curseStatus: string;
  creationDate: Date;
...
async function queryGraphQl(query: string) {
  const response = await fetch(
    'https://api.studio.thegraph.com/query/44302/sarcotest2/17',
    {
      method: "POST",
...
export class SubgraphData {
  ...
  > => {
    try {
      ...
      return {
        ...
        curseStatus: !archAddress ? '' : getCurseStatus(
          sarcophagusData,
          archAddress,
          blockTimestamp,
   ...
   */
  static getArchaeologistSarcophagi = async (archAddress: string): Promise<SarcophagusDataSimple[]> => {
    try {
   ...
   */
  static getArchaeologistSarcophagiIds = async (archAddress: string): Promise<string[]> => {
    try {
      const { sarcophagusDatas } = (await queryGraphQl(getArchSarcosQuery(archAddress))) as {
        sarcophagusDatas: SarcoDataSubgraph[];
      };
